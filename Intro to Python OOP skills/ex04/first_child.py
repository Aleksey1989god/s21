...
def main():
    if len(sys.argv) != 2:
        print('Usage: python first_child.py "../../datasets/data.csv"')
        sys.exit(1)
    ...
    try:
        ...
        calculations = research.Calculations(result_file_reader)
        head, tails = calculations.counts()
        print(f'{head} {tails}')
        result_head, result_tails = calculations.fractions()
        print(f'{result_head} {result_tails}')

        analytics = research.Analytics(result_file_reader)
